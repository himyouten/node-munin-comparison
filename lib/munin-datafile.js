...
var Datafile = function(datafile){
    ...
    this.parseLine = function(line){
        ...
        if (typeof this._json[group][graph] == 'undefined'){
            this._json[group][graph] = {host: host, title: '', metrics: {}};
        }
        if(metric.length > 0 && typeof this._json[group][graph].metrics[metric] == 'undefined')
        {
            this._json[group][graph].metrics[metric] = {info: '', type: '', label: ''};
        }
        switch (stat){
            case "graph_title":
                this._json[group][graph].title = text;
                break;
            case "info":
                this._json[group][graph].metrics[metric].info = text;
                break;
            case "type":
                this._json[group][graph].metrics[metric].type = text;
                break;
            case "label":
                this._json[group][graph].metrics[metric].label = text;
                break;
        }
        logger.log("debug", "parseLine:%s %s ", group, metric, this._json[group][metric]);
    };
    this.setSelectopts = function(){
        this._selectopts = { _all: [] };
        for (group in this._json){
            this._selectopts._all.push({text: group.toUpperCase()});
            this._selectopts[group] = [];
            for (graph in this._json[group]){
                for (metric in this._json[group][graph].metrics){
                    var type = this._json[group][graph].metrics[metric].type;
                    if (type.length == 0){
                        type = 'd';
                    } else {
                        type = type.substring(0,1).toLowerCase();
                    }
                    var name = this._json[group][graph].host + ' ' + this._json[group][graph].title + " "+metric;
